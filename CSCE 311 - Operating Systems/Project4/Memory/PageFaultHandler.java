...
public class PageFaultHandler extends IflPageFaultHandler
{
  ...
  public static int do_handlePageFault(ThreadCB thread, 
    ...
    if(page.isValid()) {
      return FAILURE;
    } //return Failure if page is valid
    ...
    if(newFrame.getPage() != null) { 
      ...
      if(newFrame.isDirty()) {
        ...
        if(thread.getStatus() == GlobalVariables.ThreadKill) {
          ...
          event.notifyThreads();
          ThreadCB.dispatch();
          return FAILURE; //Dispatch and return failure if thread is to be killed
        }
    ...
    if(thread.getStatus() == ThreadKill) {
      ...
      event.notifyThreads();
      ThreadCB.dispatch();
      return FAILURE; //if thread is to be killed, frame is to be null and return failure  
    }
    newFrame.setPage(page);
    page.setValid(true);
    if(newFrame.getReserved() == Task) {
      newFrame.setUnreserved(Task);      
    } // Check if frame is reserved, is so unreserve
    page.setValidatingThread(null);
    page.notifyThreads();
    event.notifyThreads();
    ThreadCB.dispatch();
    return SUCCESS; //if method completes, return success
  }
  
  // method for finding and returning new frame
